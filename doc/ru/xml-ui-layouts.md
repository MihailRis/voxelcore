...
Неинтерактивный элемент для отображения прогресса. Рисует фон и заполненную часть.

- `min` — минимальное значение. Тип: число. По-умолчанию: 0
- `max` — максимальное значение. Тип: число. По-умолчанию: 100
- `value` — начальное значение. Тип: число. По-умолчанию: 0
- `bar-color` — цвет заполненной части. Тип: RGBA цвет. По-умолчанию: синий `#3399FFCC`
- `bg-color` — цвет фона (пустой части). Тип: RGBA цвет. По-умолчанию: чёрный полупрозрачный `#00000066`
- `orientation` — направление заполнения: `horizontal` / `vertical`
- `supplier` — Lua-функция-поставщик значения, вызывается каждый кадр
- `smooth` — плавное изменение значения при обновлениях. Тип: логический. По-умолчанию: false
- `text-format` — формат текста по центру бара (sprintf, два аргумента: `%1$` — сырое значение, `%2$` — процент 0–100)
  ```xml
  ...
  ```
- `text-color` — цвет текста. Тип: RGBA цвет. По-умолчанию: белый `#FFFFFFFF`
