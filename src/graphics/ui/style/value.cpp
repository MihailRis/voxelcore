...
namespace style {
    ...
    glm::vec4 value::asColor(const glm::vec4& def) const {

        if (isVec4()) {
            return std::get<glm::vec4>(data);  // если храним glm::vec4
        }
        if (isString()) {
            // Попытка распарсить строку в цвет: "#RRGGBBAA" или "rgba(...)"
            if (auto parsed = parseColor(std::get<std::string>(data))) {
                return *parsed;
            }
        }
        return def;
    ...
    float value::asFloat(float def) const {
        ...
        if (isInteger()) {
            return static_cast<float>(std::get<int64_t>(data));
        }
        if (isString()) {
            if (auto parsed = parseFloat(std::get<std::string>(data))) {
                return *parsed;
    ...
    namespace parsers {
        std::optional<int64_t> parseInt(const std::string& str) {
            ...
            try {
                ...
                return pos == str.length() ? std::make_optional(val)
                                           : std::nullopt;
            } catch (...) {
                return std::nullopt;
            }
        ...
        switch (this->getType()) {
            ...
            case Type::String:
                return std::get<std::string>(this->data) ==
                       std::get<std::string>(other.data);
            case Type::Integer:
                return std::get<int64_t>(this->data) ==
                       std::get<int64_t>(other.data);
            case Type::Float:
                return std::abs(
            ...
            case Type::Vec2:
                return std::get<glm::vec2>(this->data) ==
                       std::get<glm::vec2>(other.data);
            case Type::Vec3:
                return std::get<glm::vec3>(this->data) ==
                       std::get<glm::vec3>(other.data);
            case Type::Vec4:
                return std::get<glm::vec4>(this->data) ==
                       std::get<glm::vec4>(other.data);
            default:
                return false;
        }
    }
