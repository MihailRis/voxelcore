...
namespace style {
    ...
    public:
        ...
        void set_inherit_exceptions(const std::vector<std::string>& props) {
            inherit_exceptions = props;
        }
    private:
        std::vector<std::string> inherit_exceptions;
        Stylesheet stylesheet;
        void compute_base(Node& node, Node& parent);
        void compute_base(Node& node);

        bool match_selector(
            Node& node, Selector& selector, Node* parent = nullptr
        );

        template <typename T>
        bool process_selector_variant(
            Node& node, const T& selector, Node* parent = nullptr
        );
        bool process_simple_selector(
            Node& node, const SimpleSelector& selector
        );
        bool process_complex_selector(
            Node& node, ComplexSelector selector, Node* parent = nullptr
        );
        bool check_pseudo_classes(Node& node, const SimpleSelector& selector);
        bool check_nth_child(
            const Element& element, const std::string& formula
        );
        bool check_nth_of_type(
            const Element& element, const std::string& formula
        );
        bool check_custom_pseudo_class(
        ...
        void extract_state_from_simple_selector(
            const SimpleSelector& selector, ElementState& state
        );
        void extract_state_from_complex_selector(
